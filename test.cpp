...
public:
    void getRentedCarData(ifstream &input, rentedCars rCars[])
    {
        ...
        int i = 1;
        while (input >> rCars[i].realId >> rCars[i].rentedId >> ws && getline(input, rCars[i].sDate) >> ws && getline(input, rCars[i].lDate) >> rCars[i].day >> ws && getline(input, rCars[i].sMonth) >> rCars[i].rent)
        {
            if (rCars[i].realId == (totalRentedCars - 1))
            {
                break;
            }
            cout<<rCars[i].getRent()<<endl;
            i++;
        }
    }
    void setId(int i)
...
void userMenu(total &obj, Users tUsers[], Car tCars[], rentedCars rCars[], string &tempEmail, Driver drivers[])
{
    ...
    cout << "2: See your previous bookings" << endl;
    cout << "3: Edit your details: ";
    cout << "4: Quit";
    int choice;
again:
    cout << "Enter your choice: ";
